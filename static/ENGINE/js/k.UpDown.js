jQuery.fn.extend(
{
    ...
    {   var self = this;
        var options = jQuery.extend({selectOnFocus:false,setFirstFocus:false},options);
    
        var $elements = false;
        if (this.is("table")) $elements = $("tbody>tr>td>input:text",self);
        else if (this.is("tr")) $elements = $("td>input:text",self);
        
        if (options.selectOnFocus)
            $elements.unbind("focus").focus(function(){$(this).select();})
        ...
        {   $elements.each(function()
                    ...
                    {   case 38: // up
                        {   var index = 0;
                            ...
                            {   var $i = $tr.find("td").eq(index).find("input:text");
                                ...
                                {   $i.get(0).focus(); 
                                    flag = false;
                                    if ( jQuery.fn.rowFocus ) $tr.parents("table").rfSetFocus($tr);
                                }
                                else
                                {   $tr = $tr.prev();
                                    flag = ($tr.length>0);
            ...
            if (options.setFirstFocus)
                $elements.eq(0).get(0).focus();
        }
