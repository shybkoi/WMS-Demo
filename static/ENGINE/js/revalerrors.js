...
jQuery.fn.RevalErrors = function(options) {    
    var options = jQuery.extend({usersonstart:false,uid: 0,displaying:false,source:''},options);        
    $("#divError").remove();
    ...
    $("#errorExcel").remove();
    $("#sel_userstart").remove();
    var ttl="Ошибки переоценки ";
    if(isOnWares) ttl+="для '"+errorWaresname+"'";
    var divError = $("<div/>")
            .attr("id","divError")
            .addClass("flora");
    if(options.displaying) {
        divError.css({"height":"430px","width":"96%","margin":"2% 0"});
    } else {
        $(divError).dialog({height:450,width:750,modal:true,resizable:true,draggable:true,title:ttl,overlay:{backgroundColor:'#000',opacity: 0.5}});
    ...
    msec += 1000*60*60*24;
    dend = new Date(msec);
    mnth = dend.getMonth()+1;
    dtcur=dend.getDate()<10?"0"+dend.getDate():dend.getDate();
    mnth=mnth<10?"0"+mnth:mnth;
    dEnd=dtcur+"."+mnth+"."+dend.getFullYear();
    dateEnd.datepicker().mask('99.99.9999').val(dEnd);
    ...
    var searchBut = $("<button />")
        ...
        .click(function(){
            ...
            var days = parseInt(DaysBetweenDates($('#errorDateBegin').val(),$('#errorDateEnd').val()));
            if($("#sel_userstart").is(":visible") && $("#sel_userstart option:selected").val().toString()=="0" && days>1) {
                alert("Период выбора при опции 'Все пользователи' не может превышать 1 день");
                return;
            }
            $("#tableErrors").remove();
            ...
            $("#tableErrors").kTblScroll().Scrollable("320","100%").rowFocus();//kScrollableToDown({parent: '#divError'}).tablesorter().rowFocus();
        });
    if(options.usersonstart) {
        ...
        $.ajax({
            ...
            success: function(JSON) {
                if(JSON.data && JSON.data.length > 0){                   
                    for(var i=0;i<JSON.data.length;i++){
                        if(options.uid && JSON.data[i].ID_USER==options.uid){
        ...
        if(isOnWares) {        
            dHeader.append("<br/>По товару <span style='color: green;'>"+errorWaresname+"</span>");
        }
    ...
    if(options.displaying){
        $("#"+options.source).append(divError);
    } else
    ...
    function DaysBetweenDates(strbeg, strend){
        return ((StrToDate(strend)-StrToDate(strbeg))/(24 * 60 * 60 * 1000));
    }
