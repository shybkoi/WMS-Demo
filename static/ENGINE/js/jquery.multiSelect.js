...
//
//		1.0.2	- Fixed issue where dropdown doesnt scroll up/down with keyboard shortcuts
...
if(jQuery) (function($){
	...
	$.extend($.fn, {
		multiSelect: function(o, callback) {
			...
			if( o.optionAttrs == undefined ) {o.optionAttrs = false;}            
			// Initialize each multiSelect
			$(this).each( function() {
				var select = $(this);
				var html = '<input type="text" readonly="readonly" class="multiSelect" id="input-'+$(select).attr("id")+'" value="" style="cursor: default;" />';
				html += '<div class="multiSelectOptions" id="'+$(select).attr("id")+'" style="position: absolute; z-index: 99999; display: none; width:'+$(select).width()+'px;">';
				...
				$(select).find('OPTION').each( function() {
					if( $(this).val() != '' ) {
                        ...
                        if(o.optionAttrs!=false) {
                            for(var i=0;i<o.optionAttrs.length;i++) {
                                item = o.optionAttrs[i];
                                html += item + '="' + $(this).attr(item) + '" ';
					...
					if( $(this).next('.multiSelectOptions').is(':visible') ) {
						...
						if( e.keyCode == 13 || e.keyCode == 32 ) {
							...
							if( $(this).next('.multiSelectOptions').find('LABEL.hover INPUT:checkbox').attr('checked') ) {
								...
								// Select all status cant be checked at this point
		...
		multiSelectUpdateSelected: function(o) {
			...
			if( i == 0 ) {
                ...
                $(this).prev('INPUT.multiSelect').val($(optn).find('INPUT:checkbox:checked').not('.selectAll').eq(0).closest("label").text());
            } else {
