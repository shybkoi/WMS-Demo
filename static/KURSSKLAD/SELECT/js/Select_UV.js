    ...
    function tblClientDocs(JSON){
        ...
        function tblDocToSelect(JSON,manyDocs) {
            ...
            if (JSON.ext_data.ALGO=='W'){                    
                ...
                html = '<table><thead>'+
                                '<tr><th colspan=5>Задание</th>\
                                     <th colspan=3>Товар</th>\
                                     <th colspan=4>К отборке</th>\
                                     <th colspan=2>Остаток</th>\
                                     <th colspan=3>Категория</th></tr>\
                                <tr><th ksort=digit>№</th><th ksort=text>Зона МП</th><th ksort=text title="Сегмент местоположений">СМП</th><th ksort=text title=Метод>М</th><th ksort=digit title=Позиция>П</th>\
                ...
                for (var i=0; i<JSON.data.length; i++){
                    ...
                    html += '<tr>'+
                                ...
                                '<td class=text title="' + w.SSGNAME + '">'+w.SSGCODE+'</td>'+
                ...
                html += '</tbody><tfoot><tr><th colspan=17 class=buttons>';
            }
        ...
        function trHTML(json){
            return  $.tdDocStatus(json.STAT, json.STATNAME) +
              ...
              (saleThroughName ? '<td class=throuhg>' + json.THROUGHNAME + '</td>' : '')  +
