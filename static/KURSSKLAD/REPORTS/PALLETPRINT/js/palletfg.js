$(document).ready(function () {
  ...
  function data(json){
    ...
    var html = '<table><thead><tr>' + 
        ...
        '<th ksort="text" title="Производственная линия">Линия</th>' +   
        '<th ksort="text" title="Производитель">Производитель</th>' +
        '<th ksort="digit" title="Код товара">Код</th>' +
        '<th ksort="text" title="Наименование товара">Наименование</th>' +
        '<th ksort="date" title="Дата производства">ДП</th>' +
        '</tr></thead><tbody>';
    var cntPallets = 0;    
    for (var i=0; i<json.data.length; i++){
        ...
        html += '<tr>' +
            ...
            '<td class="text">' + tr.PLNAME + '</td>' +
            '<td class="text">' + tr.PRODUCER + '</td>' +
            '<td class="number">' + tr.WCODE + '</td>' +
            '<td class="text">' + tr.WNAME + '</td>' +
            '<td>' + kDate(tr.PRDATE) + '</td>' +
          '</tr>';
    }
    html += '</tbody><tfoot><tr><th>' + cntPallets + '</th><th>' + json.data.length + '</th><th colspan=7>&nbsp;</th></tfoot></table>';
