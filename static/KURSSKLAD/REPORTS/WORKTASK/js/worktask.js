$(document).ready(function(){   
    ...
    $('#btnPrint').click(function() {
        if (!$('#tblWorkers').length) {showMes('Внимание','Нет данных для печати!');return false;}
...
//;(function($) {
                        ...
                        if ($('#tblWorkers>thead>tr:first>th[ttid="'+ttid+'"]:first').length) {
                            ...
                            if (!json.data[i].NOUSETASKTIME)
                                $td.next().text(kIntervalToTime(json.data[i].SUMTIME))
                                    ...
                                    .next().text(kInt(json.data[i].CNTUNITS));
                        }
                        else {
                            var colspan = (!json.data[i].NOUSETASKTIME ? '6' : '4');
                            $("#tblWorkers>thead>tr")
                                ...
                                .filter(':last').append('<th title="Количество заданий" ksort=digit>КЗ</th>'+
                                                        ...
                                                        '<th>Вес</th><th>Объем</th><th title="Количество единиц">КЕ</th>');
    ...
    function Workers() {
        ...
        if (!params.ttid) {showMes('Внимание','Не выбраны типы заданий!'); return false;}
