    ...
    success: function (json) {
      if (!showErr(json)) {
        html_systems = '<table id="tblSystems"><thead><tr>\
                                        <th class="chk"><input type="checkbox" value=""></th>\
                                        <th>Наименование</th>\
                                    </tr></thead><tbody>';
        for (var i = 0; i < json.data.length; i++) {
          html_systems += '<tr>\
                                        <td class="chk"><input type="checkbox" value="' + json.data[i].ID_USER + '" ' + (json.data[i].ID_USER.length ? '' : 'disabled') + '>\
...
(function ($) {
  ...
  $.WorkersTTDetail = function () {
    ...
    var params = {};
    var users = $('#btnSystems').attr('users').split(',').unique().join(',')
    params.timebeg = timebeg;
    params.timeend = timeend;
    params.users = users;
    if (!params.users) {
      showMes('Внимание', 'Не выбраны интерфейсы!');
      return false;
    }
    ...
    function calcTask(data, index, $progressbar) {
      ...
      function addTaskTypeColumn(data, index) {
        ...
        var l = 1;
        if (index != 'sum' && !data.NOUSETASKTIME) l = l + 2;
    ...
    function calcTaskSum($progressbar) {
      ...
      if ($.globalTimer) {
        $.globalTimer = setTimeout($.WorkersTTDetail, $.globalTimerInterval);
      }
    }
  };
