$(document).ready(function() {
    ...
    $("#frmFilter").bind("submit",function() {
        ...
        $.getJSON("report",params,$.tblWaresNoStorageLife);
		return false;
	});
    $("#btnPrint").click(function(){
        var wnd = window.open(sp_reports+'/print.html');
        wnd.onload = function() {
            wnd.document.getElementById('tbl').innerHTML = $("#tblWaresNoStorageLife").printHTML();
        }
    });
});
...
;(function($) {
    ...
    $.tblWaresNoStorageLife = function(JSON){
        if (!showErr(JSON)) {
			var html = '<table id=tblWaresNoStorageLife><thead><tr><th colspan=4>Товар</th><th colspan=2>Остаток по документам</th><th colspan=2>Остаток по партиям</th></tr>\
