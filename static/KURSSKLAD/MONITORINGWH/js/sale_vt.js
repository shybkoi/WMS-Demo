...
;(function($){
    ...
    function tblDocsSale(JSON){
        ...
        function tr(JSON){
            return $.tdDocStatus(JSON.DOCSTATUS)+
                   ...
                   '<td class="text">'+JSON.FROMOBJNAME+'</td>'+
        ...
        if ($tbl.length==0){                
            var html = '<table id="tblDocsSale"><thead>'+
                            '<tr><th>Ст</th><th>ШК</th><th>От кого</th><th>Кому</th><th>Дата</th><th>Номер</th><th>Сумма</th><th>ПТ</th></tr>'+
                        '</thead><tbody>';            
            for (var i=0; i<JSON.data.length; i++) 
                html += '<tr id="'+trM+kInt(JSON.data[i].DOCID)+'">'+tr(JSON.data[i])+'</tr>';
            html += '</tbody><tfoot><tr><th colspan="8">Итого: '+JSON.data.length+'</th></tr></tfoot></table>';
    ...
    function tblTaskSel(JSON){
        ...
        if ($tbl.length==0) {                
            var html = '<table id="tblDocsSale"><thead>'+
                            '<tr><th colspan="8">Документ</th><th colspan="3">Задание</th></tr>'+
                            '<tr><th>Ст</th><th>ШК</th><th>От кого</th><th>Кому</th><th>Дата</th><th>Номер</th><th>Сумма</th><th>ПТ</th><th>Ст</th><th>Номер</th><th>Исполнитель</th></tr>'+
                        '</thead><tbody>';            
            for (var i=0; i<JSON.data.length; i++) 
                html += '<tr id="'+trM+kInt(JSON.data[i].TASKID)+'">'+tr(JSON.data[i])+'</tr>';
            html += '</tbody><tfoot><tr><th colspan="11">Итого: '+JSON.data.length+'</th></tr></tfoot></table>';
