...
$(document).ready(function(){
    $("#frmDocFilter-chkDate").change(function(){
                ...
                $.getJSON('docTypeOptions',{doctid:JSON.ext_data.doctid,viewdays:1,chkamountsign:1},function(JSON){
                    if (!showErr(JSON)){
                        ...
                        for (var i=0; i<JSON.data.length; i++){
							...
							var flagIsObj = JSON.data[i].ISOBJZONE || JSON.data[i].ISOBJRESERVE;
							var url = '';
							if (JSON.data[i].ISOBJZONE){
								url = 'ajaxGetObjects';
							}
							else if (JSON.data[i].ISOBJRESERVE){
								url = 'ajaxGetObjectsReserve';
							}
							
							switch(flagIsObj) {
								case 's': {
									var html='<select id="'+$obj.attr("id")+'">'
									$.ajax({url:url,success:function(JSONOBJ){
								...
								case 'i': {
									...
									$('#'+id).unbind('click').kObjAutoComplete({hiddenName:id,action:url});
                            ...
                            $('#'+id).attr('isobjreserve',JSON.data[i].ISOBJRESERVE);
                        }
    ...
    $("#cmbDocType").cmbSpDocTypes({
        withoutFilter:false,
        onChange:docTypeChg,
        funcOptAttr: function(data){
            return ' code="' + data.CODE + '"' +
                ' wsetid="' + data.WSETID + '"' +
                ' pricecode="' + data.PRICECODE + '"' +
                ' initstatus="' + data.FIRSTSTATUS + '"' +
                ' cancreate="' + data.CANCREATE + '"' +
                ' canedit="' + data.CANEDIT + '"' +
                ' restrict="' + data.DOCMANAGEREDITDISABLE +'"';
        },
    });
                ...
                //$.ajax({url:'getBUYRETURN',success:function(JSON2){
                    $("<ul/>").attr("id",mId).addClass("contextMenu").css("width","190px")
                        .html('<li class="add "><a href="#docAdd">Добавить</a></li>'+
                              ...
                              '<li class="information"><a href="#docReserve">Резерв</a></li>'+
    ...
    function tblDetail(JSON){
        ...
        function trHTML(cg){
            return  '<td waresid="' + cg.WID + '"><input type="checkbox"></td>' +
                    ...
                    (useviewunit ? ('<td title="' + viewTitle(cg.MUC, cg.VUF, cg.VUC) + '">' + viewQuantity(cg.AMOUNT, cg.VUF, cg.VUC, cg.MUF, cg.MUC) + '</td>') : '') +
                    ...
                    '<td class="number brutto">' + kFloat(cg.BRUTTO, 1) + '</td>';
        };
        ...
        if (!showErr(JSON)){        
            if ($("#tblDetail").length){
                ...
                for (var i=0; i<JSON.data.length; i++){
                    ...
                    if ($tr.length) {
                        addSum -= kFloat($tr.find(">td.docsum").text());
                        addBrutto -= kFloat($tr.find(">td.brutto").text());
                ...
                var $th = $("#tblDetail>tfoot>tr>th.docsum");
                $th.text( kFloat(kFloat($th.text())+addSum,4));
                var $th = $("#tblDetail>tfoot>tr>th.brutto");
                $th.text( kFloat(kFloat($th.text())+addBrutto,1));				
                ...
                var html = '<table id="tblDetail"><thead><tr>' + 
                    ...
                    '<th>Код</th>' + 
                    '<th>Наименование</th>' +
                    '<th title="Единица измерения">ЕИ</th>' +
                ...
                html += '</tbody><tfoot><tr><th>'+JSON.data.length+'</th>'+
                            ...
                            '</th>' +
                            '<th class="docsum">' + kFloat(docsum, 4) + '</th>' + 
                            '<th class="brutto">' + kFloat(brutto, 1) + '</th>' + 
                            '</tfoot></table>';
        ...
        if (!showErr(JSON)){
                ...
                if($('#cmbDocType option:selected').attr('initstatus') != $('#tblDocs').rf$GetFocus().attr('status'))    
                    $('#tblDetail a.edit').click(function(){
                        //console.log(canEdit.call($('#tblDocs').rf$GetFocus()));
...
//-------------------------------------------------------------------------------------------------------------------------------------
    function $docDialog(dvOptions,docOptions){
        ...
        var from = $('#fromobj').attr('isobjzone') || $('#fromobj').attr('isobjreserve');
        switch (from) {
            case 's':
            {
            ...
            case 'i':
            {
                ...
                $('#docFrom').unbind('click').kObjAutoComplete({hiddenName: id, action: 'ajaxGetObjects'});
        ...
        var through = $('#throughobj').attr('isobjzone') || $('#throughobj').attr('isobjreserve');
        switch (through) {
            case 's':
            {
            ...
            case 'i':
            {
                ...
                $('#docThrough').unbind('click').kObjAutoComplete({hiddenName: id, action: 'ajaxGetObjects'});
        ...
        var to = $('#toobj').attr('isobjzone') || $('#toobj').attr('isobjreserve');
        switch (to) {
            case 's':
            {
            ...
            case 'i':
            {
                ...
                $('#docTo').unbind('click').kObjAutoComplete({hiddenName: id, action: 'ajaxGetObjects'});
                valCopy('toobj', 'docTo');
                break;
            }
            default:
    ...
    function canEdit(message) {
        ...
        if($('#cmbDocType option:selected').attr('canedit') == '0' ||
            $('#cmbDocType option:selected').attr('restrict') == '1' ||
            $('#cmbDocStatus option[value="'+status+'"]').attr('restrict') == '1') {
            if(!message)
    ...
    function canCreate(message) {
        var status = $(this).attr('status');
        if ($('#cmbDocType option:selected').attr('cancreate') == '0') {
            if (!message)
                alert('Документ создать нельзя!');
            return false;
        }
        return true;
    }
    ...
    function docReserve(){
		if ($("#dvDocReserve").length) 
			$("#dvDocReserve").dialog("destroy").remove();

        var $dv = $("<div/>").attr("id","dvDocReserv")
                .addClass("flora").css({'overflow-y': 'auto'})
                .dialog({closeOnEscape: true, title: 'Изменения резерва', autoOpen: true,
                         resizable: false, draggable:false, modal:true, overlay: {opacity:0.5,background:"black"},
						 height: $(document.body).height() * 0.9, width: $(document.body).width() * 0.9
						});
						
        $dv.whReserveSummary({
          urlReserveSummary: "qReserveDocSummary",
          params: {docid:$(this).ID()},
          urlRestWaresList: false,
          urlRestWaresData: false,
          urlReserveLvl1Hist: 'qReserveDocLvl1Hist',
          urlReserveLvlDoc: false,
          urlReserveLvl2Hist: 'qReserveDocLvl2Hist',
          urlReserveLvl2Clients: 'qReserveDocLvl2Clients'
        });
    };
                                    ...
                                    if (!showErr(JSON)){
                                        ...
                                        for (var i=0; i<JSON.data.length; i++) {
                                            ...
                                            html += '<tr>'+
                                                        ...
                                                        u('<td class="number" title="'+viewTitle(tr.MUC,(tr.VUF/tr.MUF),tr.VUC)+'">'+viewQuantity(tr.AMOUNT,tr.VUF,tr.VUC,tr.MUF,tr.MUC)+'</td>')+
                                                        '<td class="number">'+((tr.MUC=='шт.') ? kFloat(tr.AMOUNT,0):kFloat(tr.AMOUNT,3))+'</td>'+
        ...
        function frmSubmit(){
            $.getJSON("cargoDel",{cargoid:ID($("#tblDetail").rfGetFocus())},function(JSON){
                if (!showErr(JSON)){
                    ...
                    if ($tr.length){
                        var $th = $tr.parents("table:first").find("tfoot>tr>th.docsum");
                        $th.text( kFloat(kFloat($th.text()) - kFloat($tr.find("td.docsum").text()),4));
                        var $th = $tr.parents("table:first").find("tfoot>tr>th.brutto");
                        $th.text( kFloat(kFloat($th.text()) - kFloat($tr.find("td.brutto").text()),1));
