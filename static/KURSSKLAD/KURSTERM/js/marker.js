...
function handleMarkerKeyDown(evt){
  ...
  var isSelect = node.tagName=='SELECT';
  ...
  if (getKeyCode(evt) == 37 || getKeyCode(evt) == 9 && evt.shiftKey) {
    ...
    if (isSelect){
        step = 0;
        if (is_ie){
            var selectedIndex = parseInt(node.selectedIndex,10);
            if (selectedIndex > 0) node.selectedIndex = selectedIndex-1;
            CancelAction(evt);
        }    
    }
  ...
  if (getKeyCode(evt) == 39 || getKeyCode(evt) == 9 && !evt.shiftKey) {
    ...
    if (isSelect){
        ...
        if (is_ie){
            var selectedIndex = parseInt(node.selectedIndex,10);
            if (selectedIndex < node.length - 1) node.selectedIndex = selectedIndex+1;
            CancelAction(evt);
        }        
    }
