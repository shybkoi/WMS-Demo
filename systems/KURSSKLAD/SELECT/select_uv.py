...
from systems.KURSSKLAD.SELECT.templates.main_uv import main_uv as tmplmain
import conf.client_conf as clConf
from kinterbasdb import DatabaseError as FBExc
import db

class SelectUV(WHCommon):
    ...
    def docSelectPreview(self, docid):
        try: 
            ...
            if d['ALGO'] and d['ALGO']=='W':
                ...
                data = self.dbExecC(sql='select * from UV_TASKSELECT_CREATE_PREVIEW(?)',params=[docid], fetch='all')
        except FBExc, exc: return self.pyDumps(ext_data = {'errMes':exc[1]})
