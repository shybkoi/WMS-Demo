...
class SpWares(WHCommon,KSprav):
    ...
    def waresGetUBD(self, wid):
        try: w = self.dbExec(sql='select WUSEBYDATE,WUBTCODE,WUBTNAME from K_WH_SPWARES_WDATA(?)',params=[wid],fetch='one')
        except FBExc, exc: return self.pyDumps({'errMes':exc[1]})
        return self.pyDumps(data=w,ext_data={'WID':wid})
    waresGetUBD.exposed = True

    @logSet('waresSetUBD')
    def waresSetUBD(self, wid, ubdVal, ubdType):
        'Изменение срока годности'
        try: self.dbExec(sql='update gwares g set g.usebydate = ?,g.ubdtype = ?,g.sessionid = Null where g.waresid = ?',params=[ubdVal,ubdType,wid],fetch='none')
        except FBExc, exc: return self.pyDumps({'errMes':exc[1]})
    ...
    def addWUBarcode(self,wuid,barcode):
        try: 
            self.dbExec(sql='insert into waresbarcode values (?,?,?,?,?)',params=[None,wuid,barcode,None,None],fetch='none')
    ...
    def chgWUBarcode(self,barcid,barcode):
        try: self.dbExec(sql='update waresbarcode wbr set wbr.barcode = ? where waresbarcodeid = ?',params=[barcode,barcid],fetch='none')
        except FBExc, exc: return self.pyDumps({'errMes':exc[1]})
